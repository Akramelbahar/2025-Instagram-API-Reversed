...
class CryptoPub:
    ...
    def encrypt_native(version: int, public_key_pem: str, data: str, nonce: str) -> bytes:
        ...
        Args:
            version (int): Version number (e.g., 41)
...
def login(username , actual_password):
    try:
        ...
        headers = {
            ...
            "user-agent": "Instagram 409.1.0.41.113 Android (; INFINIX/Infinix; Infinix ; Infinix; mt6781; en_US; )",
        ...
        open('text.txt' , "w").write(json.dumps(data))
        ...
        try:
            ...
            headers = {
                ...
                "authorization": dataDict["IG-Set-Authorization"],
                "ig-u-ds-user-id": decode_base64_to_dict(dataDict["IG-Set-Authorization"].replace(" ","").replace("Bearer","").split(":")[-1]).get("ds_user_id"),
                "ig-intended-user-id": decode_base64_to_dict(dataDict["IG-Set-Authorization"].replace(" ","").replace("Bearer","").split(":")[-1]).get("ds_user_id"),
                "content-type": "application/x-www-form-urlencoded; charset=UTF-8",
                "accept-encoding": "gzip, deflate",
            }
            data = f"normal_token_hash=&device_id=android-{deviceUid.split('-')[0] if deviceUid.split('-')[0] else '69c742fc0b549ac3'}&_uuid={dataDict["dataDict"]}&custom_device_id={dataDict["dataDict"]}&fetch_reason=token_expired"
            ...
            data = {"bool_opt_policy":"0","mobileconfigsessionless":"","api_version":"3","unit_type":"1","query_hash":"","device_id":f"{deviceUid}","fetch_type":"ASYNC_FULL","family_device_id":"EMPTY_FAMILY_DEVICE_ID"}
            response = requests.post("https://b.i.instagram.com/api/v1/zr/dual_tokens/" , headers=headers  ,data=data)
            content = response.content
            encoding = response.headers.get('Content-Encoding', '')
            try:
...
class C204589Su:
    ...
    def a00(i: int, i2: int, j: int) -> Optional[str]:
        ...
        Args:
            i: First integer parameter
            i2: Second integer parameter
            j: Long parameter
        
        Returns:
            Encoded string or None if there's an error
        """
        try:
...
def comment(authorization, mediaId, message, accountUid, deviceUid , _uuid):
    ...
    headers = {
        ...
        "ig-u-ds-user-id": decode_base64_to_dict(authorization.replace(" ","").replace("Bearer","").split(":")[-1]).get("ds_user_id"),
        "ig-intended-user-id": decode_base64_to_dict(authorization.replace(" ","").replace("Bearer","").split(":")[-1]).get("ds_user_id"),
        "content-type": "application/x-www-form-urlencoded; charset=UTF-8",
        "accept-encoding": "gzip, deflate",
    }
    print(headers)
    time.sleep(5)
    url = "https://i.instagram.com/api/v1/nudges/generate_nudge/"
    data=f"is_bottom_sheet_open=true&media_id={mediaId}&source=commenting&_uuid={mediaId}&viewed_comment_ids=[]"
    r = requests.post(url , headers=headers ,data=data)
    print(r.text)
    url = "https://i.instagram.com/api/v1/media/comment/check_offensive_comment/"
    payload = {"media_id":mediaId,
               "_uid":accountUid.replace(",",""),
               "comment_session_id":decode_base64_to_dict(authorization.replace(" ","").replace("Bearer","").split(":")[-1]).get("sessionid"),
               "_uuid":_uuid,
...
#comment("3469466130684359876_67027536123","this is a text i want to write" , "54586931999" , "fd1522f9-2663-4efd-89c8-435d6e988ddb" )
...
def testExplore(authorization ,accountUid , deviceUid ):
    ...
    try:
        ...
        print(content)
    return False
def explore(authorization , accountUid , deviceUid ,_uuid, max_id=0 , session_paging_token="" , session_id="" , paging_token=""):
    max_id = max_id +1
    ...
    headers = {
        ...
        "authorization": authorization,
        "x-mid": "ZyfOYQABAAEAfwYam8CdJAjb-QTw",
        "ig-u-ig-direct-region-hint": f"FRC",
        "ig-u-ds-user-id": accountUid,
        "ig-intended-user-id": accountUid,
        "content-type": "application/x-www-form-urlencoded; charset=UTF-8",
        "accept-encoding": "zstd, gzip, deflate",
    ...
    try:
        ...
        for id in ids:
                ...
                comment(authorization , f"{id}",cm , accountUid ,deviceUid ,_uuid )
...
def explorebySearch(authorization , accountUid , deviceUid ,_uuid,searchTerm  ,  page_index=0 , reels_max_id="" , next_max_id="" , rank_token=""  , page_token="" , paging_token={"total_num_items":4}):
    try:    
        ...
        try:
            ...
            explorebySearch(authorization , accountUid , deviceUid ,_uuid,searchTerm  , page_index+1 , reels_max_id , next_max_id , rank_token , page_token , paging_token)
        except json.JSONDecodeError:
            print("Content is not JSON. Raw data:")
            print(content)
    except Exception as e :
...
actual_password = ""
username = "elbahar.exe" # DONT FORGET TO FOLLOW
username = ""
actual_password = ""
...
lg = (login("elbahar.exe" , ""))

#print(lg)
explore( lg["IG-Set-Authorization"] , lg["accountId"].replace(",","") , lg["deviceUid"] ,lg["uuid"] )
#explorebySearch(lg["IG-Set-Authorization"] , lg["accountId"] , lg["deviceUid"] ,"a")
...
#print(comment(lg["IG-Set-Authorization"],"3460681155839541797_60519778850","rwi7a mol tmrma" , lg["accountId"] , lg["deviceUid"],lg["uuid"] ))
